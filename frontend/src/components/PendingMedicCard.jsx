...
export const PendingMedicCard = ({ medico, onApprove, onReject }) => {
  ...
  const handleDownloadPDF = () => {
    if (medico.cv_pdf_url) {
      const link = document.createElement('a');
      link.href = medico.cv_pdf_url;
      link.download = `CV_${medico.id_medico}.pdf`;
        ...
        <div className="p-6">
          ...
          <div className="grid grid-cols-2 gap-4 mb-4 text-sm">
            ...
            <div className="col-span-2">
              ...
              <p className="text-gray-700 font-semibold">{medico.numero_colegiado || medico.no_colegiado || 'N/A'}</p>
            </div>
          </div>
          ...
          {medico.cv_pdf_url && (
            <motion.button
              ...
              className="w-full flex items-center justify-center gap-2 bg-purple-50 text-purple-600 font-semibold py-2 px-3 rounded-lg hover:bg-purple-100 transition border border-purple-200 mb-3"
            >
              <MdDescription className="text-lg" />
              <span>Ver CV Profesional</span>
            </motion.button>
          )}
            ...
            >
              ...
              <div className="flex justify-between items-center p-4 border-b">
                <h3 className={`${Size.LARGE} font-bold text-gray-800`}>
                  CV Profesional - Dr(a). {medico.nombre} {medico.apellido}
                </h3>
                <button
                  onClick={() => setShowPdfModal(false)}
